...
export default async function DealDetailPage({ params }: DealDetailPageProps) {
  ...
  const userVote = session?.user?.id
    ? (deal.votes.find((vote) => vote.userId === session.user.id)?.voteType as "UP" | "DOWN" | "CONFIRM" | "EXPIRED" | undefined)
    : undefined;
  ...
  const restaurantWebsiteHref = getSafeExternalHref(deal.restaurant.website);
  const mapPoints = hasCoordinates(deal.restaurant)
    ? [
        {
          id: deal.id,
          title: deal.title,
            ...
            <div className="mt-8 grid gap-5 md:grid-cols-2">
              <Card variant="muted" className="p-5">
                ...
                <div className="mt-4 space-y-2 text-sm leading-6 text-gray-300">
                  <p className="font-medium text-white">{deal.restaurant.name}</p>
                  <p>{getRestaurantAddress(deal.restaurant)}</p>
                  ...
                  <div className="flex flex-wrap gap-3 pt-2">
                    <a
                      href={getRestaurantMapsHref(deal.restaurant)}
                      target="_blank"
                      rel="noreferrer"
